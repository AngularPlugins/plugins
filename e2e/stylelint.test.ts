...
describe.skip('Stryker', () => {
  it('should be able test projects using stylelint', async done => {
    ...
    runAngularCLICommand(
      'sample',
      'generate @angular-plugins/stylelint:stylelint-project sample'
    );
    //   expectMutationFail(
    //     await runCommandAsync('sample', 'yarn ng run sample:stylelint', {
    //       silenceError: true
    //     })
    //   );
  ...
  it.skip('should be able test projects using stylelint', async done => {
    // create a new AngularCLI app
    ...
    expectMutationFail(
      await runCommandAsync('sample', 'yarn ng run sample:mutate', {
        silenceError: true
      })
    );
